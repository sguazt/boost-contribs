...
BOOST_UBLAS_TEST_DEF( check_close )
{
	...
	// Check T vs. T
	BOOST_UBLAS_DEBUG_TRACE( "-- Test against same types." );
	BOOST_UBLAS_TEST_CHECK_CLOSE( float(1), float(1), tol );
	BOOST_UBLAS_TEST_CHECK_CLOSE( double(1), double(1), tol );
	BOOST_UBLAS_TEST_CHECK_CLOSE( ::std::complex<float>(1,2), ::std::complex<float>(1,2), tol );
	BOOST_UBLAS_TEST_CHECK_CLOSE( ::std::complex<double>(1,2), ::std::complex<double>(1,2), tol );

	// Check T1 vs. T2
	BOOST_UBLAS_DEBUG_TRACE( "-- Test against different types." );
	BOOST_UBLAS_TEST_CHECK_CLOSE( float(1), double(1), tol );
	BOOST_UBLAS_TEST_CHECK_CLOSE( double(1), float(1), tol );
	BOOST_UBLAS_TEST_CHECK_CLOSE( ::std::complex<float>(1,2), ::std::complex<double>(1,2), tol );
	BOOST_UBLAS_TEST_CHECK_CLOSE( ::std::complex<double>(1,2), ::std::complex<float>(1,2), tol );

	// Check alias
	BOOST_UBLAS_DEBUG_TRACE( "-- Test aliases." );
	BOOST_UBLAS_TEST_CHECK_PRECISION( float(1), float(1), tol );
}
...
BOOST_UBLAS_TEST_DEF( check_rel_close )
{
	...
	// Check T vs. T
	BOOST_UBLAS_DEBUG_TRACE( "-- Test against same types." );
	BOOST_UBLAS_TEST_CHECK_REL_CLOSE( float(1), float(1), tol );
	BOOST_UBLAS_TEST_CHECK_REL_CLOSE( double(1), double(1), tol );
	BOOST_UBLAS_TEST_CHECK_REL_CLOSE( ::std::complex<float>(1,2), ::std::complex<float>(1,2), tol );
	BOOST_UBLAS_TEST_CHECK_REL_CLOSE( ::std::complex<double>(1,2), ::std::complex<double>(1,2), tol );

	// Check T1 vs. T2
	BOOST_UBLAS_DEBUG_TRACE( "-- Test against different types." );
	BOOST_UBLAS_TEST_CHECK_REL_CLOSE( float(1), double(1), tol );
	BOOST_UBLAS_TEST_CHECK_REL_CLOSE( double(1), float(1), tol );
	BOOST_UBLAS_TEST_CHECK_REL_CLOSE( ::std::complex<float>(1,2), ::std::complex<double>(1,2), tol );
	BOOST_UBLAS_TEST_CHECK_REL_CLOSE( ::std::complex<double>(1,2), ::std::complex<float>(1,2), tol );

	// Check alias
	BOOST_UBLAS_DEBUG_TRACE( "-- Test aliases." );
	BOOST_UBLAS_TEST_CHECK_REL_PRECISION( float(1), float(1), tol );
}
...
BOOST_UBLAS_TEST_DEF( check_matrix_close )
{
	...
	ublas::matrix<float> fA = ublas::scalar_matrix<float>(nr, nc, 1);
	ublas::matrix<float> dA = ublas::scalar_matrix<float>(nr, nc, 1);
	ublas::matrix< ::std::complex<float> > cfA = ublas::scalar_matrix< ::std::complex<float> >(nr, nc, ::std::complex<float>(1,2));
	ublas::matrix< ::std::complex<double> > cdA = ublas::scalar_matrix< ::std::complex<double> >(nr, nc, ::std::complex<double>(1,2));
